          ...
          <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="space-y-4">
              {sessions?.map((session) => (
                <div key={session.id} className="rounded-xl border border-stone-200 p-4">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">
                    {session.caregiver_label || "Anonymous caregiver"} · {new Date(session.created_at).toLocaleString()}
                  </p>
